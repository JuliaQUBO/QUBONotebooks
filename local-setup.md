...
The target installs `docs`, `qubo`, and `cudaq` and runs both optional sections
on `qpp-cpu`, with QPU access forced off:

- The QUBO notebook prepares a hand-written QAOA circuit for its original
  11-variable penalty model, optimizes its expectation with SciPy, and compares
  finite-shot samples with exhaustive enumeration. The actual Hamiltonian
  diagonal is checked against all 2048 binary costs. Raw Ising energy, QUBO
  cost, original objective, probability, and sample frequency are kept explicit.
- The D-Wave notebook evolves its existing 11-variable QUBO, verifies both
  Hamiltonian endpoints, compares samples with enumeration, and plots the
  spectrum and optimal-solution probability versus anneal time. The normalized
  times are illustrative, not hardware microseconds.
...
109 seconds on Linux x86_64 (WSL2), using Python 3.12, CUDA-Q 0.16, and two CPU
threads. Runtime varies with the machine; the CI budget remains 10 minutes
including installation.

The full QUBO notebook also requires GLPK (`glpsol`) for its earlier
integer-programming comparisons; install it using that notebook's setup
instructions. The optional CI job installs `glpk-utils` before verification.
...
The QUBO notebook publishes the skip path; its QAOA results are generated by
the opt-in target in `.nbverify/2-QUBO_python.ipynb`. The D-Wave notebook's
published availability message, numerical results, and figure come from one
CUDA-Q-enabled CPU run. This difference is deliberate: [#140](https://github.com/JuliaQUBO/QUBONotebooks/issues/140)
requests a committed skip notice for QAOA, while the D-Wave lesson keeps its
previously published enabled example. Missing-package tests and portable
notebook targets verify the skip paths. The CUDA-Q target requires both
optimization sections to complete their numerical checks; a missing dependency
cannot silently skip. Run `make test-cudaq-python` for the focused kernel,
mapping, and import-guard tests in the same CPU environment.
To run only one lesson, override `CUDAQ_PYTHON_NOTEBOOKS`, for example
`make verify-cudaq-python CUDAQ_PYTHON_NOTEBOOKS=notebooks_py/2-QUBO_python.ipynb`.
